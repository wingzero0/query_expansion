...
class RecommendationTree{
	...
	private $nTmpTb; // tmp table for n-gram
	private $ngramTb; // n-gram table
	private $trees;
	public function __construct($relateTb, $tmpNgramTb, $ngramTb){
		$this->rTb = $relateTb;
		$this->nTmpTb = $tmpNgramTb;
		$this->ngramTb = $ngramTb;
		$this->trees = null;
	}
	...
	private function ConstructTrees($relatedQ, $depth) {
		$trees = array();
	...
	public function NgramIDF($flag) {
		...
		if ($flag == DB){
			...
			$sql = sprintf(
				"SELECT count(distinct(`query`)) FROM `%s`", $this->nTmpTb
			);
			$result = mysql_query($sql) or die($sql."\n".mysql_error());
